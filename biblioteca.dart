...
class Biblioteca{
  ...
  void removeLivros(){
    ...
    for (var livro in livros){
      if(aux==index-1){
        print("\nTem certeza?");
        print('O livro selecionado foi: titulo: ${livro.titulo}, autor: ${livro.autor}');
      }
      aux++;
    }  
    print("1. Sim 2. Não");
  ...
  void emprestarLivro(){
    ...
    for (var livro in livros){
      if(aux==index-1){
        print("\nTem certeza?");
        print('O livro selecionado foi: Título: ${livro.titulo}\nAutor: ${livro.autor}');
      }
      aux++;
    }  
    print("1. Sim 2. Não");
    ...
    if(op==1){
      ...
      emprestimo.add(Cliente(name,livros.removeAt(index-1),DateTime.now(),DateTime.now().add(Duration(minutes: 1)),this.status(DateTime.now(),DateTime.now().add(Duration(minutes: 2))))); 
  ...
  void devolverLivro(){
    ...
    for (var livro in emprestimo){
      ...
      String format=DateFormat('dd/MM/yyyy HH:mm:ss').format(livro.dataEmprestimo);
      print("Data Emprestimo: ${format}");
      print('Título: ${livro.emprestimo.titulo}\nAutor: ${livro.emprestimo.autor}');
      String format2=DateFormat('dd/MM/yyyy HH:mm:ss').format(livro.dataEntrega);
      print("Data Entrega: ${format2}");
      print("Data atual: ${DateFormat('dd/MM/yyyy HH:mm:ss').format(DateTime.now())}");
    ...
    do{
      ...
      index=int.parse(stdin.readLineSync()!);
      if(index>emprestimo.length){
         print("\nIndice invalido!");
      }
    }while(index>emprestimo.length); 
    Livro temp=Livro('','');
    for (var livro in emprestimo){
      
      if(aux==index-1){
        print("\nTem certeza?");
        print('O livro selecionado foi: \nTitulo: ${livro.emprestimo.titulo}\nAutor: ${livro.emprestimo.autor}');
  ...
  void consultarEmprestimo(){
    ...
    for (var livro in emprestimo){
      ...
      String format=DateFormat('dd/MM/yyyy HH:mm:ss').format(livro.dataEmprestimo);
      print("Data Emprestimo: ${format}");
      print('Titulo: ${livro.emprestimo.titulo}\nAutor: ${livro.emprestimo.autor}');
      String format2=DateFormat('dd/MM/yyyy HH:mm:ss').format(livro.dataEntrega);
      print("Data Entrega: ${format2}");
      print("Data atual: ${DateFormat('dd/MM/yyyy HH:mm:ss').format(DateTime.now())}");
      livro.status=this.status(DateTime.now(), livro.dataEntrega);
